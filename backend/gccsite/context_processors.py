def my_context_processor(req):
    ...
    privacy_list = [
        ...
        },
        {
            ...
            "content": f"""<h4>L'essentiel</h4><p>Vous disposez (le cas échéant <em>via</em> le responsable légal) des droits d'accéder et d'obtenir copie des données vous concernant, de vous opposer au traitement de ces données, de les faire rectifier ou les faire effacer. Vous disposez également d'un droit à la limitation du traitement de vos données. <a href="https://www.cnil.fr/fr/comprendre-vos-droits">Rendez-vous sur le site de la CNIL pour mieux comprendre vos droits.</a></p><h4>Exercer vos droits</h4><p>Toute demande d'exercice de vos droits doit être adressée par voie électronique à <a href="mailto:{context['CONTACT_EMAIL']}">{context['CONTACT_EMAIL']}</a>.</p><h4>Réclamation auprès de la CNIL</h4><p>Si vous estimez, après nous avoir contacté, que vos droits sur vos données ne sont pas respectés, vous pouvez alors <a href="https://www.cnil.fr/fr/webform/adresser-une-plainte">adresser une réclamation à la CNIL</a>.</p>""",
        },
    ]
    faq_entries = [
        {
