...
from events.models import Address, Center, Event
...
def get_centers(centers):
    for center in centers:
        ...
        address = Address(
            ...
            city=center["city"],
            zip_code=int(center["postal_code"])
...
def main(file_path):
    ...
    with transaction.atomic():
        ...
        for event in get_events(data["events"], centers_by_id):
            ...
            event.save()
        for application in get_applicants(
            data["applicants"], events_by_id, profiles_by_user_id
        ):
            application.save()
